...
│           └── md/route.ts               # GET — 导出摄影书 Markdown 源文件 (zip)
├── analytics/
...
│       ├── markdown.ts                   # Markdown Export —— Book Source Format
...
published 摄影集支持两种导出格式：PDF（最终出版物）和 Markdown（可编辑源文件）。
...
BookDocument (中间结构)
       ↓
resolveBookImages()          ← schema.ts (加载图片 base64，仅 PDF)
       ↓
┌──────────────┬──────────────────┐
...
- **layout.ts**：页面尺寸、边距、排版常量（仅 PDF renderer 使用）
- **pdf.tsx**：PDF renderer（只是 BookDocument 的一个渲染目标）
- **markdown.ts**：Markdown exporter（从 BookDocument 生成内容，不包含版式）
- **types.ts**：BookDocument / Page 类型定义

未来扩展（IDML / EPUB / Web Book）只需新增 renderer，复用 `buildBookDocument()`。
...
Markdown Export 与 PDF Export 的根本区别：

| | PDF | Markdown |
|------|------|------|
| **定位** | 最终出版物 | 可编辑源文件 |
| **包含** | 版式、字体、页码、边距 | 仅内容（标题、版本、照片、说明） |
| **图片** | base64 嵌入（优化后 3000px JPEG） | 原文件直接复制到 images/ |
| **比例** | 4:5 / 1:1 / 3:2 / 2:3 | 无（Markdown 没有页面尺寸概念） |
| **输出** | .pdf 单文件 | .zip (book.md + images/) |
| **API** | `/api/collections/[id]/export` | `/api/collections/[id]/export/md` |

**设计原则：** Markdown 是 Book Source Format——不含版式、不含比例、不含页码。这些属于 PDF Renderer。因此 Markdown 导出不存在 4:5 / 1:1 / 3:2 / 2:3 等比例概念。
...
├── book.md
└── images/
    ├── Tokyo 2026_v3_001_DSCF0001.JPG
    ├── Tokyo 2026_v3_002_DSCF0005.RAF
    └── Tokyo 2026_v3_003_R0001055.JPG
```
...
![Tokyo 2026_v3_001_DSCF0001.JPG](images/Tokyo 2026_v3_001_DSCF0001.JPG)

抵达东京后的第一个傍晚。

---

![Tokyo 2026_v3_002_DSCF0005.RAF](images/Tokyo 2026_v3_002_DSCF0005.RAF)

上野公园。
```

### Export Image Naming Policy

导出图片采用 `{collection_title}_v{version}_{pageIndex:03d}_{original_filename}` 命名。

**目的：**
1. 保持摄影书阅读顺序（pageIndex 三位补零 → Finder 中自然排序）
2. 快速定位原始素材（原文件名保留 → 可搜索、可关联）
3. 避免多个摄影集导出后重名（标题 + 版本作为前缀）
4. 保留原始扩展名（不统一转换 → Markdown 是源文件格式，不是出版物）

**非法字符处理：** `/ \ : * ? " < > |` 替换为 `-`，空格保留。跨平台兼容。

## Collection Management

摄影集支持三种管理操作：修改名称、批量移除照片、删除摄影集。

### 修改名称

- 位置：摄影集详情页标题旁铅笔图标
- 点击 → 进入内联编辑 → Enter 保存 / Esc 取消
- API：`PATCH /api/collections/[id]` with `{ title }`

### 批量移除照片

与首页共用 Selection Mode：
- 点击「选择照片」进入多选模式
- 勾选照片 → 工具栏显示「已选择 N 张 · [移出摄影集] · 取消」
- API：`DELETE /api/photos/batch` (context: collection)
- 行为：collection_id → NULL，照片回到首页瀑布流

### 删除摄影集

- 位置：摄影集详情页标题行右侧「删除摄影集」按钮
- 点击 → 确认对话框 → 不可撤销
...
| 摄影集记录 | 永久删除 |
| 照片文件 | 不受影响 |
| 照片记录 | 保留（collection_id → NULL，回到首页） |
| 备注 / EXIF | 保留 |
| 已导出 PDF / Markdown | 不受影响（属于用户本地文件，系统不追踪） |

**设计原则：** 删除摄影集 ≠ 删除照片。摄影集只是照片的组织方式，删除组织方式不影响内容。已导出文件属于用户磁盘，系统不追踪、不删除。

### API

| 方法 | 端点 | 作用 |
|------|------|------|
| `PATCH` | `/api/collections/[id]` | 更新摄影集（含 title） |
| `DELETE` | `/api/collections/[id]` | 删除摄影集（解除关联 + 删除记录） |

## Collection Chapters

摄影集支持轻量级章节功能。章节是内容结构，不是版式结构。

### 设计原则

- 章节是内容层概念，不是排版层概念
- PDF 和 Markdown 基于同一个章节数据源生成
- 章节和照片共享统一的排序系统（sort_order 交叉排列）
- 不实现目录、章节封面、章节模板等复杂功能

### 数据模型

`collection_chapters` 表：`id`, `collection_id`, `title`, `sort_order`。与 `photos.sort_order` 交叉排列形成统一的内容流。

### UI

- 摄影集编辑模式下显示「+ 新增章节」按钮
- 点击后输入章节标题，Enter 确认
- 已有章节以 pill 标签形式展示，可删除
...
| 方法 | 端点 | 作用 |
|------|------|------|
| `POST` | `/api/collections/[id]/chapters` | 新增章节 |
| `DELETE` | `/api/collections/[id]/chapters?id=` | 删除章节 |

## Chapter Navigation

摄影集编辑视图新增章节导航栏，基于现有 sort_order + chapter 结构实现，不新增数据库字段。

### 设计原则

章节导航用于**快速定位和编辑**，不是分类管理。出口逻辑不变。

### 章节照片统计

照片计数基于 sort_order 区间：章节之后、下一章节之前的所有照片。`sort_order IS NULL` 的照片不归入任何章节。

### 交互

| 操作 | 行为 |
|------|------|
| 点击章节 pill | 章节高亮 2 秒（ring 样式） |
| 再次点击同一章节 | 进入 Focus Mode |
| 点击「全部」 | 退出 Focus Mode，显示全部照片 |
...
首页和摄影集详情页支持多选模式，共用同一个 Selection Mode。选择照片后执行批量操作，操作完成后自动退出选择模式。
...
而不是为每种操作创建独立的模式。批量删除、批量加入摄影集、批量移出——都在同一个选择模式下完成。
...
| 操作 | 按钮样式 | 可用位置 | API |
|------|---------|---------|-----|
| 加入摄影集 | 蓝色主按钮 | 首页 | `PATCH /api/photos/batch` |
| 删除 | 红色危险按钮 | 首页 + 摄影集 | `DELETE /api/photos/batch` |
| 移出摄影集 | 红色危险按钮 | 摄影集内 | `DELETE /api/photos/batch` (context: collection) |

### 批量加入摄影集

1. 选择照片 → 点击「加入摄影集」
2. 弹出摄影集选择器（仅显示 draft / ready，不显示 published）
3. 选择目标摄影集 → 确认
4. 批量 PATCH → 照片从首页瀑布流移除 → 退出选择模式

**为什么禁止导入 published 摄影集：** 已发布摄影集视为已出版内容，冻结不可修改。必须先撤回发布（published → ready）才能导入新照片。保持摄影书版本稳定性。
...
| 方法 | 端点 | Body | 作用 |
|------|------|------|------|
| `PATCH` | `/api/photos/batch` | `{ photo_ids, collection_id }` | 批量加入摄影集 |
| `DELETE` | `/api/photos/batch` | `{ photo_ids, context }` | 批量删除 / 移出 |

### 实现文件

- `components/PhotoCard.tsx` — checkbox UI
- `components/PhotoGrid.tsx` — 多选管理 + 批量操作工具栏
- `app/api/photos/batch/route.ts` — PATCH + DELETE 批量接口
- `lib/db.ts` — `batchAddToCollection()` / `batchDeletePhotos()` / `batchRemoveFromCollection()`
...
| `DELETE` | `/api/collections/[id]` | 删除摄影集（解除所有照片关联，照片回到首页） |
| `GET` | `/api/collections/[id]/export` | 导出摄影书 PDF（根据 book_ratio 生成相应尺寸） |
| `GET` | `/api/collections/[id]/export/md` | 导出摄影书 Markdown 源文件（zip：book.md + images/） |
